...
| 機能 | stokado | store2 | store.js | lscache | localForage | idb-keyval | @vueuse/useStorage |
|------|---------|--------|----------|---------|-------------|------------|--------------------|
| Proxy シンタックスシュガー | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| 型安全なシリアライゼーション | ✅ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ |
| 同一タブ内リアクティブサブスクライブ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ |
...
> 凡例：✅ ネイティブ対応；◐ JSON レベルのみ（プリミティブは可、Date/RegExp/Function/undefined の型は失われる）；❌ 非対応。
...
stokado は localStorage ラッパー、ブラウザストレージプロキシ、Web ストレージライブラリです。localStorage シリアライゼーション、ストレージリアクティビティ、ストレージサブスクリプション、ストレージ有効期限、クロスタブストレージ同期、非同期ストレージサポート、ストレージクォータアラートを提供します。store2、store.js、lscache、localForage、idb-keyval の代替。localStorage、sessionStorage、localForage、および任意の storage ライクオブジェクトで動作します。
